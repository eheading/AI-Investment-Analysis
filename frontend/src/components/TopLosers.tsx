...
export default function TopLosers() {
  ...
  const fetchStocks = useCallback(async (m: Market) => {
    ...
    setError(null);
    try {
      const data = await api.getTopLosers(m);
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const result = await api.analyzeTopLosers(market, symbols);
  ...
  const handleMoneyFlow = async () => {
    ...
    setError(null);
    try {
      const result = await api.analyzeLosersMoneyFlow(market);
        ...
        <div className="mt-6 rounded-xl border border-[#1e1e2e] bg-[#0d0d15] p-6">
          <h3 className="mb-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <SaveToStoryButton source="top_losers_analysis" title={`Top Losers AI Analysis (${market})`} content={analysis} defaultMarket={market} />
              <TranslateToggle
      ...
      {moneyFlow && !analyzingFlow && (
        <div className="mt-6 rounded-xl border border-amber-800/30 bg-[#0d0d15] p-6">
          <h3 className="mb-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <SaveToStoryButton source="top_losers_money_flow" title={`Top Losers Money Flow (${market})`} content={moneyFlow} defaultMarket={market} />
              <TranslateToggle
